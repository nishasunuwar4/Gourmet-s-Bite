...
$R_IDsql = "SELECT RESTAURANTS.R_ID FROM RESTAURANTS WHERE RESTAURANTS.M_ID='$user_check'";
$R_IDresult = mysqli_query($conn, $R_IDsql);
$R_IDrs = mysqli_fetch_array($R_IDresult, MYSQLI_BOTH);

if (!$R_IDrs) {
    // No restaurant found for the manager, show error message
    ?>
    ...
    if (!$success) {
        echo "<p>There was an error adding the food item. Please try again later.</p>";
    } else {
